...
mislabeled_by_justice = {'AScalia':[], 'AMKennedy':[], 'CThomas':[],'RBGinsburg':[], 'SGBreyer':[], 'JGRoberts':[], 'SAAlito':[], 'SSotomayor':[],'EKagan':[]}
...
#	train_cols = ['caseOriginState','issue','issueArea','lcDispositionDirection','caseSource']
	train_cols = ['issue','issueArea','lcDispositionDirection','petitioner','respondent']
	df_with_labels = df[ df['direction'].isin([1,2]) ]
	df_with_labels = df_with_labels[train_cols+['direction','docket']]
...
#	model_B = BaggingClassifier(model)
	...
	for model,model_name in [(model_RF,"random forest"),(model_Logistic,"logistic"),(model_KN,"nearest neighbor")]:
		avg_total = 0
		baseline_total = 0
...
#                print test_with_labels['issueArea'].value_counts() / len(test_with_labels)
			...
			mislabeled_by_justice[judge] = mislabeled_by_justice[judge]+ mislabeled['docket'].tolist()
		...
		print judge
		print model_name,avg_total/5
		print "baseline",baseline_total/5
		print avg_total/5 - baseline_total/5
		break
...
for j in ['AScalia', 'AMKennedy', 'CThomas','RBGinsburg', 'SGBreyer', 'JGRoberts', 'SAAlito', 'SSotomayor','EKagan']:
	predict_judge(j)
	print '-----------------------'
#a= set(mislabeled_by_justice['AScalia'])
a= set(mislabeled_by_justice['RBGinsburg'])
#for j in ['AMKennedy', 'CThomas','RBGinsburg', 'SGBreyer', 'JGRoberts', 'SAAlito', 'SSotomayor','EKagan']:
#for j in ['CThomas','JGRoberts', 'SAAlito']:
for j in ['SGBreyer','SSotomayor','EKagan']:
	a = a & set(mislabeled_by_justice[j])
print len(a)
print a
